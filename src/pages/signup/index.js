			...
			<div className="row justify-content-center">
				<div className="col-11 col-md-8">
					<form onSubmit={handleSubmit(onSubmit)} className={styles.form} autoComplete="off">
						<Input {...register('name', { required: true, maxLength: 45 })} placeholder="نام" />
						{errors.name?.type === 'required' && <Error field="نام" type="required" />}
						{errors.name?.type === 'maxLength' && <Error field="تعداد کاراکتر های نام" type="maxLength" />}
						...
						/>
						{errors.phone?.type === 'required' && <Error field="تلفن همراه" type="required" />}
						{errors.phone?.type === 'pattern' && <Error field="تلفن همراه" type="pattern" />}
						{errors.phone?.type === 'maxLength' && <Error field="تلفن همراه" type="phoneNumberLength" />}
						{errors.phone?.type === 'minLength' && <Error field="تلفن همراه" type="phoneNumberLength" />}
						...
						/>
						{errors.password?.type === 'required' && <Error field="رمز عبور" type="required" />}
						...
						{errors.password?.type === 'invalidInformation' && <Error type="invalidInformation" />}
